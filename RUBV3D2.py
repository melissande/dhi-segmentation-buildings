...
class inconv(nn.Module):
    """
    initial residual layer, 1st stage of expansion path for the U-Net model
    """
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.batch_norm:
            out = self.bn2(out)
#         if self.dropout > 0: #to add for more regularization
#             out = self.drop(out)
        if self.downsample:
...
class up_residual(nn.Module):
    ...
    def __init__(self, in_ch, out_ch,width_kernel, dropout=0,batch_norm=False):
        ...
        #self.up = nn.ConvTranspose2d(in_ch, out_ch, 2, stride=2)#to use up convolutions instead of bilinear upsampling
        self.conv = make_residual_layer(ResidualBlock, in_ch, out_ch,1, width_kernel,dropout=dropout,batch_norm=batch_norm)
    ...
    def forward(self, x1, x2):
        ...
        x = torch.cat([x2, x1], dim=1)#residual connection
        x = self.conv(x)
        return x
...
class UNet(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, depth = 3, n_features_zero = 32, width_kernel=3,dropout=0,batch_norm=True,distance_net=True,bins=10):
        """
        initialize the model
        Args:
            in_channels (int): number of input channels (panchromatic+pansharpened=9)
